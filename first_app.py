...
st.write("Follow this link to see the complete flow of the simulation: https://drive.google.com/file/d/1jDIaXIps10KcuU_oRdTScoSpBo9zc4DY/view?usp=sharing")
st.write("\n")
st.write("\n")
...
# INPUT VARIABLES
duration = st.sidebar.number_input("Duration of the simulation in days", step = 1, min_value = 1) # number of days to run the simulation for

pallets_per_day = st.sidebar.number_input("Number of arriving pallets per day", step = 10, min_value = 10)

waiting_manpower = 9999999 # pseudo process which must always happen when called upon, so, unlimited capacity
checking_manpower = st.sidebar.number_input('Number of employees working for: checking (a checker both checks and labels)', step = 0.5, min_value = 0.5)
putaway_manpower = st.sidebar.number_input('Number of employees working for: put away', step = 0.5, min_value = 0.5)
...
red_line = st.number_input("Y-axis position of red line", step = 1, min_value = 2)
yellow_line = st.number_input("Y-axis position of yellow line", step = 1, min_value = 1)
...
ax.set_title("Temporary stock in expedition")
st.pyplot(fig)

fig, ax = plt.subplots(2)
fig.suptitle("Unloaded temporary stock, not yet checked \n Checked and labelled temporary stock, not yet put away")
